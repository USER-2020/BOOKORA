...
export default function Business({ draft = {}, currentStep = 1 }) {
    const [step, setStep] = useState(Math.min(currentStep, 8));
    ...
    const { data, setData, post, processing, errors } = useForm({
        step: 1,
        business_name: draft.business_name || '', business_category: draft.business_category || '', business_description: draft.business_description || '', country: draft.country || 'Colombia', city: draft.city || '',
    ...
    const next = () => {
        if (step === 8) { setProvisioning(true); window.localStorage.setItem('noryvaq_demo_profile', JSON.stringify({ ...data, created_at: new Date().toISOString() })); document.cookie = 'noryvaq_demo=1; max-age=2592000; path=/; SameSite=Lax'; [1, 2, 3].forEach((stage) => setTimeout(() => setProvisionStage(stage), stage * 550)); setTimeout(() => post(route('onboarding.complete'), { onError: () => setProvisioning(false), onFinish: () => setProvisioning(false) }), 1700); return; }
        setData('step', step);
    ...
    return <div className="min-h-screen bg-[#f7f8fc] text-slate-900"><Head title={'Onboarding · ' + steps[step - 1]} /><header className="flex h-16 items-center justify-between border-b border-slate-200 bg-white px-5 lg:px-8"><a href="/" className="flex items-center gap-3"><span className="flex h-9 w-9 items-center justify-center rounded-xl bg-[#5b50f5] text-lg font-black text-white">N</span><span className="font-bold tracking-tight">Noryvaq</span></a><span className="hidden text-xs font-medium text-slate-400 sm:block">Reserva. Conecta. Haz crecer.</span><span className="text-xs font-semibold text-slate-500">Guardado automáticamente</span></header><div className="mx-auto grid max-w-[1500px] gap-6 p-4 sm:p-6 lg:grid-cols-[220px_minmax(0,1fr)_280px] lg:p-8">
        ...
        {step === 7 && <div className="grid gap-5 sm:grid-cols-2"><div><InputLabel value="Nombre" /><TextInput value={data.first_name} onChange={(e) => setData('first_name', e.target.value)} className="mt-1 block w-full" required /></div><div><InputLabel value="Apellido" /><TextInput value={data.last_name} onChange={(e) => setData('last_name', e.target.value)} className="mt-1 block w-full" required /></div><div><InputLabel value="Correo" /><TextInput type="email" value={data.email} onChange={(e) => setData('email', e.target.value)} className="mt-1 block w-full" required /></div><div><InputLabel value="Teléfono" /><TextInput value={data.phone} onChange={(e) => setData('phone', e.target.value)} className="mt-1 block w-full" /></div><div><InputLabel value="Cargo" /><TextInput value={data.role} onChange={(e) => setData('role', e.target.value)} className="mt-1 block w-full" /></div><div><InputLabel value="Contraseña de acceso" /><TextInput type="password" minLength="8" value={data.password} onChange={(e) => setData('password', e.target.value)} className="mt-1 block w-full" required /></div><div className="sm:col-span-2"><FieldSelect label="Objetivo principal" value={data.objective} onChange={(value) => setData('objective', value)} options={['Recibir más reservas', 'Organizar mi agenda', 'Reducir trabajo manual', 'Cobrar online', 'Gestionar varias sedes', 'Gestionar personal', 'Mejorar la experiencia del cliente', 'Centralizar mi operación', 'Otro']} /><FieldSelect label="Modalidad de demo" value={data.demo_mode} onChange={(value) => setData('demo_mode', value)} options={[{ value: 'try_now', label: 'Probar Noryvaq ahora' }, { value: 'guided', label: 'Demo guiada con nuestro equipo' }, { value: 'both', label: 'Quiero ambas opciones' }]} /></div></div>}
        {step === 8 && <div className="rounded-3xl bg-gradient-to-br from-indigo-50 to-cyan-50 p-6"><div className="mb-5 flex h-14 w-14 items-center justify-center rounded-2xl bg-[#5b50f5] text-2xl text-white">✓</div><h2 className="text-xl font-bold">Tu espacio de prueba está listo.</h2><p className="mt-2 text-sm text-slate-600">Configuramos Noryvaq según la forma en que funciona tu negocio.</p><div className="mt-6 grid gap-3 sm:grid-cols-2">{['Perfil del negocio creado', 'Modelo de reserva configurado', 'Módulos activados', 'Dashboard preparado', 'Datos demo generados'].map((item) => <div key={item} className="text-sm font-medium text-slate-700">✓ {item}</div>)}</div></div>}
...
function ProvisioningOverlay({ stage }) {
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-[#11184b]/70 p-5 backdrop-blur-sm">
