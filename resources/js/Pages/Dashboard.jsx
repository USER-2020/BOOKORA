        ...
        <AuthenticatedLayout>
            ...
            <div className="min-h-[calc(100vh-4rem)] bg-[#f7f8fc] px-4 py-8 sm:px-6 lg:px-8">
                <div className="mx-auto max-w-7xl">
                    ...
                    </> : <section className="rounded-3xl border border-slate-200 bg-white p-8 text-center shadow-sm"><div className="mx-auto flex h-14 w-14 items-center justify-center rounded-2xl bg-indigo-50 text-[#5b50f5]"><Icon name={navigation.find(([label]) => label === active)?.[1] || 'spark'} size={26} /></div><h2 className="mt-5 text-xl font-bold text-slate-950">{active}</h2><p className="mx-auto mt-2 max-w-md text-sm leading-6 text-slate-500">Esta sección está lista para trabajar con la configuración de <strong>{business?.name}</strong>. Tu demo ya reconoce los módulos que seleccionaste.</p><button type="button" onClick={() => setActive('Inicio')} className="mt-6 rounded-xl bg-[#5b50f5] px-5 py-3 text-sm font-semibold text-white hover:bg-[#4840d2]">Volver al inicio</button></section>}
                </div>
            </div>
        </AuthenticatedLayout>
    );
